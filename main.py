...
class BlogPost(db.Model):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True,foreign_keys=[Users.id])
...
def login():
    ...
    if form.validate_on_submit():
        ...
        result=db.session.execute(db.select(form).where(form.email==email))
        # Note, email in db is unique so will only have one result.
        user=result.scalar()
        p_w=check_password_hash(user.password,password)
        if not user:
